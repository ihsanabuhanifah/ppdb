...
import { soalBahasainggris } from "./soalTesBahasaInggris";
...
export default function TestBahasaInggris() {
  const [waktu, setWaktu] = React.useState(localStorage.getItem("ti"));
  ...
  async function onSubmit(jawaban) {
    ...
    for (let i = 0; i < soalBahasainggris.length; i++) {
      let soal = `soal${i + 1}`;
      if (soalBahasainggris[i].answer === jawaban[soal]) {
        nilai = nilai + 1;
      }
    }
  ...
  React.useEffect(() => {
    let timeMatematika = localStorage.getItem("ti");
    if (timeMatematika === null || timeMatematika <= 0) {
      localStorage.setItem("ti", 600);
    }
  }, []);

  React.useEffect(function () {
    let intervalId = setInterval(() => {
      let sisaWaktu = localStorage.getItem("ti");
      
      setWaktu(sisaWaktu);
      let sisa = sisaWaktu - 1;

      localStorage.setItem("ti", sisa);
    }, 1000);
      ...
      <div className="py-10 pl-5 lg:pl-48 pr-5 lg:pr-32 container  h-full">
        ...
        >
          {({
            ...
            {waktu <= 0 ? ("") : ( soalBahasainggris.map((soal, index) => (
                <div
